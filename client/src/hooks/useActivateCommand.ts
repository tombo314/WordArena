...
interface Params {
  ...
  friendFieldIntervalRef: MutableRefObject<ReturnType<typeof setInterval> | null>;
  enemyFieldIntervalRef: MutableRefObject<ReturnType<typeof setInterval> | null>;
  friendRegenIntervalRef: MutableRefObject<ReturnType<typeof setInterval> | null>;
  enemyRegenIntervalRef: MutableRefObject<ReturnType<typeof setInterval> | null>;
  ...
  giveSlipDamage: (damage: number, target: FriendOrEnemy) => ReturnType<typeof setInterval>;
...
export function useActivateCommand(p: Params) {
  const activateCommand = (command: string, side: FriendOrEnemy) => {
    ...
    const activeField = side === "friend" ? p.activeFriendFieldRef.current : p.activeEnemyFieldRef.current;
    const disabledRef = side === "friend" ? p.disabledFriendFieldsRef : p.disabledEnemyFieldsRef;

    const isTopLevel = command in p.commandDataRef.current;
    const activeFieldData = activeField ? (p.commandDataRef.current[activeField] as Record<string, unknown>) : null;
    ...
    if (command === "regenerate") {
      ...
      if (coolTimeSec >= 0) p.coolTime.generateShieldCoolTime(coolTimeSec, side);
    } else {
    ...
    if (command === "attack" || command === "heal") {
      ...
      if (SLIP_DAMAGE_FIELDS.includes(command)) {
        const intervalRef = side === "friend" ? p.friendFieldIntervalRef : p.enemyFieldIntervalRef;
      ...
      if (side === "friend") { p.setActiveFriendField(command); p.activeFriendFieldRef.current = command; }
      else { p.setActiveEnemyField(command); p.activeEnemyFieldRef.current = command; }
      ...
      if (command === "burn out" || command === "earth quake") {
        ...
        if (side === "friend") {
          p.setActiveFriendField(null); p.activeFriendFieldRef.current = null;
          p.disabledFriendFieldsRef.current.add(activeField!);
          p.setDisabledFriendFields([...p.disabledFriendFieldsRef.current]);
        } else {
          p.setActiveEnemyField(null); p.activeEnemyFieldRef.current = null;
        ...
        const regenRef = side === "friend" ? p.friendRegenIntervalRef : p.enemyRegenIntervalRef;
        const setActiveRegen = side === "friend" ? p.setActiveFriendRegen : p.setActiveEnemyRegen;
        ...
        const defense = cmdData.defense as number;
        if (defense > 0) {
          if (side === "friend") { p.defenseFriendRef.current += defense; p.friendShieldDefenseRef.current += defense; }
          else { p.defenseEnemyRef.current += defense; p.enemyShieldDefenseRef.current += defense; }
        }
      } else {
