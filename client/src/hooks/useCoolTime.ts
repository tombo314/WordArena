...
export function useCoolTime() {
  ...
  const generateCoolTime = (coolTimeSec: number, side: FriendOrEnemy) => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (side === "friend") { inCoolTimeFriendRef.current = false; setCoolTimeFriendText(""); }
        else { inCoolTimeEnemyRef.current = false; setCoolTimeEnemyText(""); }
      }
      remaining--;
    }, 1000);
  };
  ...
  const generateRegenCoolTime = (coolTimeSec: number, side: FriendOrEnemy) => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        clearInterval(interval);
        if (side === "friend") { inRegenCoolTimeFriendRef.current = false; setRegenCoolTimeFriendText(""); }
        else { inRegenCoolTimeEnemyRef.current = false; setRegenCoolTimeEnemyText(""); }
      }
      remaining--;
    }, 1000);
  };
  ...
  const generateShieldCoolTime = (coolTimeSec: number, side: FriendOrEnemy) => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (side === "friend") { inShieldCoolTimeFriendRef.current = false; setShieldCoolTimeFriendText(""); }
        else { inShieldCoolTimeEnemyRef.current = false; setShieldCoolTimeEnemyText(""); }
      }
      remaining--;
    }, 1000);
  };
